...
var searchSelectedGenre = function (genre) {
  ...
  var watchModeGenreUrl =
    ...
    genre +
    "&limit=10&apiKey=WIu3mU2xnsXe9BTf7WlTqfAmFnw3uwR5kTG1RtbB";
...
var displayGenreResults = function (titles) {
  ...
  for (var i = 0; i < titles.titles.length; i++) {
    ...
    for (let i = 0; i < titles.titles.length; i++) {
      ...
      dynaTitleCard.innerHTML +=
        '<div id="title-card" class="card"><h3 class="card-header" "movie-title" id="selection-title' +
        i +
        '"></h3><span class="year" id="release-year' +
        i +
        '"></span><button id="details-button' +
        i +
      ...
      document.getElementById("selection-title" + i).textContent =
        titles.titles[i].title;
      document.getElementById("release-year" + i).textContent =
        titles.titles[i].year;
      document.getElementById("details-button" + i).textContent =
        "Title Details";
      document
        .getElementById("details-button" + i)
        .setAttribute("title-id", titles.titles[i].id);
    }
    break;
  }
};
...
console.log(detailsButtonClickHandler);
