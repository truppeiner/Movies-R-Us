var displaySelectedTitle = function (genre) {
  ...
  var selectedTitleApiUrl =
    ...
    selectedTitleId +
    "/details/?append_to_response=sources&apiKey=3vWxYKPiq0kntxPSw8B2hMi3OiBvlMzGkqZPSj6R";
    ...
    .then(function (response) {
      ...
      if (response.ok) {
        response.json().then(function (data) {
          ...
          const disneyPlus = sources.filter(
            (source) => source.source_id === 372
          );
          if (disneyPlus.length >= 1) {
            let disneyTitleUrl = disneyPlus[0].web_url;
            console.log(disneyTitleUrl);
            document.getElementById("streaming-links").innerHTML +=
              '<a href="" target="_blank" id="disney-plus-link"><h4 id="disney-plus">Disney+</h4></a>';
            document
              .getElementById("disney-plus-link")
              .setAttribute("href", disneyTitleUrl);
          }
          ...
          if (netflix.length >= 1) {
            ...
            document.getElementById("streaming-links").innerHTML +=
              '<a href="" target="_blank" id="netflix-link"><h4 id="netflix" >Netflix</h4></a>';
            document
              .getElementById("netflix-link")
              .setAttribute("href", netflixTitleUrl);
          }
          ...
          if (hulu.length >= 1) {
            ...
            document.getElementById("streaming-links").innerHTML +=
              '<a href=" " target="_blank" id="hulu-link"><h4 id="hulu">Hulu</h4></a>';
            document
              .getElementById("hulu-link")
              .setAttribute("href", huluTitleUrl);
          }

          const amazonPrime = sources.filter(
            (source) => source.source_id === 26
          );
          if (amazonPrime.length >= 1) {
            let amazonPrimeTitleUrl = amazonPrime[0].web_url;
            document.getElementById("streaming-links").innerHTML +=
              '<a href=" " target="_blank" id="amazon-link"><h4 id="amazon-prime">Amazon Prime Video</h4></a>';
            document
              .getElementById("amazon-link")
              .setAttribute("href", amazonPrimeTitleUrl);
          }
          ...
          if (hboMax.length >= 1) {
            ...
            document.getElementById("streaming-links").innerHTML +=
              '<a href="" target="_blank" id="hbo-max-link"><h4 id="hbo-max">HBO Max</h4></a>';
            document
              .getElementById("hbo-max-link")
              .setAttribute("href", hboTitleUrl);
          }
            ...
            if (response.ok) {
              response.json().then(function (data) {
                ...
                var cast =
                  ...
                  ", " +
                  Object.values(data.cast[2])[4] +
                  ", " +
                  Object.values(data.cast[3])[4] +
                  ", " +
                  Object.values(data.cast[4])[4] +
                  ", and " +
                  Object.values(data.cast[5])[4];
                console.log(cast);
